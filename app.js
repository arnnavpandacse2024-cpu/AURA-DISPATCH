...
function _showSyncIndicator(status) {
  ...
  if (status === 'ok')      dot.style.background = '#22c55e';
  if (status === 'syncing') dot.style.background = '#f59e0b';
  if (status === 'error')   dot.style.background = '#ef4444';
  if (status === 'loading') dot.style.background = '#64748b';
}
...
async function loadStateFromServer(silent = false) {
  try {
    ...
    if (!silent || newHash !== _lastServerHash) {
      ...
      if (silent) {
        console.log('🔄 Real-time sync: changes from another device applied.');
    ...
    if (!silent) {
      ...
      renderCourierAllocation();
      updateAgencyStats();
      // Unlock even on fallback so user can still make changes
      _stateReady = true;
    }
  }
}
...
function syncStorage() {
  ...
  fetch('/api/state', {
    ...
    body: JSON.stringify({
      shops, deliveries, totalCouriersPool,
